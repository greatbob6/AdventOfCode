...
let foldChangeDir state dir =
    ...
    elif List.head state.CurrentPath = dir && dir = "/" then
        state
    else
...
let fileMap =
    System.IO.File.ReadAllLines("7/puzzle.txt")
    |> List.ofArray
    |> parseLines
    |> List.rev
    |> generateFileMap

let findDirSize (fileMap: Map<string list, File list>) (dir: string list) =
    let fileLists =
        fileMap
        |> Map.filter (fun k v -> k[(List.length k) - (List.length dir)..] = dir)
        |> Map.toList
        |> List.map snd
        |> List.collect id
...
let getDirs (fileMap: Map<string list, File list>) =
    fileMap
    |> Map.toList
    |> List.map fst

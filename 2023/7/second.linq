...
let calcCardsValue (cards: char array) =
    let preGroupedCards =
        cards
        |> Array.groupBy id
        |> Array.map (fun (c, grp) -> (c, Array.length grp))
        |> Array.sortByDescending (fun (c, l) -> (int64 l) * 100L + (cardValue c))
        
    let remappedCards =
        if Array.exists (fun (c, l) -> c = 'J' && l <> 5) preGroupedCards then
            let jCount = preGroupedCards |> Array.find (fun (c, l) -> c = 'J') |> snd
            
            preGroupedCards |> Array.filter (fun (c, _) -> c <> 'J') |> (fun a ->
                a[0] <- (fst a[0], (snd a[0]) + jCount)
                a
            )
        else
            preGroupedCards
        
    //preGroupedCards.Dump()
    //remappedCards.Dump()
    
    let groupedCards = Array.map snd remappedCards

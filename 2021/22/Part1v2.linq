...
let pointsChunked = Seq.chunkBySize 8000000 points

let countChunk chunk =
    chunk
    |> Array.Parallel.map (fun p -> (cubeProcessor (0, p)) |> fst |> uint64)
    |> Array.sum

let totalCount =
    pointsChunked
    |> Seq.fold (fun total chunk -> total + (countChunk chunk)) 0UL

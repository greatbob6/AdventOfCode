...
let result = 
    ...
    |> Array.map (fun chars -> (parseNode chars).Node)
    |> (fun nodeArray ->
        let first = Array.head nodeArray
        let rest = Array.tail nodeArray
        
        let final = Array.fold (fun currentTree node -> addNodes currentTree node) first rest
        
        final)
    |> calculateMagnitude

result.Dump()
//(treeToString result).Dump()

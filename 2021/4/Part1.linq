...
let rec drawLoop boards drawsRemaining =
    let draw = Array.head drawsRemaining
    let updatedBoards = Array.map (updateBoard draw) boards
    let winningBoard = Array.tryFind boardWon updatedBoards
        
    match winningBoard with
    | Some board -> (board, draw)
    | None -> drawLoop updatedBoards (Array.tail drawsRemaining)
